...
interface InventorypgProps {
  setCurrentPage: (page: string) => void;
}

const Inventorypg: React.FC<InventorypgProps> = ({ setCurrentPage }) => {
  ...
  const handleInputChange = (
    ...
    if (isNewProduct) {
      setNewProduct((prev) => ({
        ...prev,
        [name]: name === "qty" || name === "price" ? numericValue : value,
        rawInput: value, // Save raw input temporarily
      }));
    } else {
  ...
  const formatOnBlur = (isNewProduct: boolean) => {
    if (isNewProduct) {
      setNewProduct((prev) => ({
        ...prev,
        rawInput: undefined, // Clear temporary raw input
        price: parseFloat(prev.rawInput?.replace(/,/g, "") || "0"), // Parse and save clean number
      }));
    } else {
      setEditingProduct((prev) =>
        prev
  ...
  // Helper function to format the price with thousand separators
   ...
   Add Mock Products
      ...
      <div className="mb-4 bg-white p-4 rounded shadow">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <input type="text" name="price" placeholder="Price" value={newProduct.price === 0 ? '' : formatPrice(newProduct.price)} onChange={ (e: React.ChangeEvent<HTMLInputElement>) => handleInputChange(e, true)} className="p-2 border rounded" />
...
>
  <FaTrash />
                ...
                <input type="text" name="name" placeholder='Name' value={editingProduct.name} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" />
                <input type="text" name="manufacturer" placeholder='Manufacturer' value={editingProduct.manufacturer} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" />
                <input type="text" name="model" placeholder='Model' value={editingProduct.model} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" />
                <input type="text" name="category" placeholder='Category' value={editingProduct.category} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" />
                {/* <input type="text" name="price" placeholder='Price' value={editingProduct.price === 0 ? '' : formatPrice(editingProduct.price)} onChange={ (e: React.ChangeEvent<HTMLInputElement>) => handleInputChange(e, true)} className="p-2 border rounded" /> */}
                <input
  type="text"
  name="price"
  placeholder="Price"
  value={editingProduct.price === 0 && !editingProduct.rawInput ? "" : editingProduct.rawInput || formatPrice(editingProduct.price)} 
  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange(e, false)}
  onBlur={() => formatOnBlur(false)} // Format when the input loses focus
  className="p-2 border rounded"
/>

                <input type="number" name="qty" placeholder='Quantity' value={editingProduct.qty === 0 ? '' : editingProduct.qty} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" />
                <input type="file" multiple onChange={handleImageChange} className="p-2 border rounded" />
                <textarea name="details" placeholder='Descriptions' value={editingProduct.details} onChange={(e) => handleInputChange(e, false)} className="p-2 border rounded" rows={3}></textarea>
...
>
  <div className="flex gap-4 w-96 overflow-x-auto"> {/* Flex container for row alignment */}
